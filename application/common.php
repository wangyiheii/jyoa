...
function applicant($id){
    ...
    $userse['name']=$user['nickname'];
    $wherese['id']=$user['department'];
    $depart=db('organization')->where($wherese)->find();
    if(!empty($user['departments'])){
        $wheresese['id']=$user['departments'];
        $departments=db('organization')->where($wheresese)->find();
        $department=$depart['name'].",".$departments['name'];
    }else{
        $department=$depart['name'];
    }
    $userse['department']=$department;
    return $userse;
...
function AuditProcess($data=""){
    ...
    $where['id']=$u_id;
    $user=db('admin')->where($where)->find();
    if($user['is_admin'] !="0"){//正式使用请吧 0  换成1 
        $controller=request()->controller();
        $action=request()->action();
        $path=$controller."/".$action;
        $where_a['url']=$path;
        $a_find=db('audit_process')->where($where_a)->find();
        if(!empty($a_find)){
...
//u_id 处理人id 
//d_id 详情id  
//is_d 详情归属 1 公司 2组织 3用户
function details_consult_log($u_id="",$d_id="",$is_d=""){
    $data['title']="查阅";
...
//d_id 详情id 
//is_d 详情归属 1 公司 2组织 3用户
function details_handle_log($title="",$content="",$u_id="",$state="",$d_id="",$is_d=""){
    $data['creation_time']=time();
    $data['type']="1";
    $data['title']=$title;
...
//content  内容
//type 
function admin_logse($admin_id,$username,$url="",$title="",$content="",$type="1"){
...
//json
function ajax_json($code="1",$msg="成功",$data=""){
    $return['code']=$code;
    $return['msg']=$msg;
    $return['data']=$data;
    $return['time']=time();
